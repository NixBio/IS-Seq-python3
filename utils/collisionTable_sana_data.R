...
wd <- '/local_scratch/ISseqOutput/230202KMFD4MAPQ30/CutAdapt/filterNo/db' 
suffixCollFile <- '230202KMFD4MAPQ30' 
previous_grouped_IS_folder <- '/local_scratch/ISseqOutput/KMFDLMAPQ30/CutAdapt/filterNo/db'
...
res <- lapply(1:n, function(f){
  
  if(file.info(files[f])$size>0){
    so=read.table(files[f],sep = "\t")
    
    x <- basename(files[f])
    x <- gsub('TEST_2','TEST-2',x)
    x <- gsub('CELL_LINE','CELL-LINE',x)
    ann=data.frame(t(unlist(strsplit(x,"_"))))
